...
{
    if( auto* ch_dwarfhelp = dynamic_cast<Dwarf*>(character) )
    {
        ch_dwarfhelp->boostHitPoints(item->getBoost() * 2);
    }
    else if( auto* ch_palhelp = dynamic_cast<Paladin*>(character) )
    {
        ch_palhelp->boostHitPoints(item->getBoost() * 1.5);
    }
    else if( auto* ch_dshelp = dynamic_cast<DragonSlayer*>(character))
    {
        ch_dshelp->boostHitPoints(item->getBoost() * 1.25);
    }
    else if( auto* ch_draghelp = dynamic_cast<Dragon*>(character) )
    {
...
{
    if( auto* ch_dwattack = dynamic_cast<Dwarf*>(character) )
    {
        ch_dwattack->boostAttackDamage(item->getBoost() * 1.5);
    }
    else if( auto* ch_paattack = dynamic_cast<Paladin*>(character) )
    {
        ch_paattack->boostAttackDamage(item->getBoost() * 1.33);
    }
    else if( auto* ch_dsattack = dynamic_cast<DragonSlayer*>(character))
    {
        assert(false);
